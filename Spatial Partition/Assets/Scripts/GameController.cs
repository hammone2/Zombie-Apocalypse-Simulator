...
namespace SpatialPartitionPattern
{
    public class GameController : MonoBehaviour
    {
        ...
        // UI Text element to show time spent in Update
        public TextMeshProUGUI timeText; // Assign this in the Inspector
        private float timeSpentInUpdate = 0f;

        //toggle stuff
        private bool isUsingSP = true;
        ...
        void Update()
        {
            // Track the time at the beginning of the update
            ...
            for (int i = 0; i < friendlySoldiers.Count; i++)
            {
                ...
                if (isUsingSP)
                    ...
                    closestEnemy = FindClosestEnemySlow(friendlySoldiers[i]); //slow ver
            ...
            // Calculate the time spent in this update
            timeSpentInUpdate = Time.realtimeSinceStartup - startTime;

            // Update the UI text with the time spent in the Update loop
            if (timeText != null)
            {
                timeText.text = "Time in Update: " + timeSpentInUpdate.ToString("F6") + " seconds";
            }
        }
